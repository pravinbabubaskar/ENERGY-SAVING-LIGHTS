 ...
 if(path) 
  {
    ...
    c_minute=now.minute();
    c_second=now.second();
    loadHandler( on_hour,  on_minute,  on_second,  off_hour,  off_minute,  off_second,  c_hour,  c_minute,  c_second); //function to handle rtc mode of project
    delay(1000);
  }
 else 
  {
...
void loadHandler(int onTimeHr, int onTimeMin, int onTimeSec, int offTimeHr, int offTimeMin, int offTimeSec,  int rtcTimeHr, int rtcTimeMin, int rtcTimeSec) //function to keep light on during on time and off during off time
{
